...
#include "TableScenario.h"

TableScenario::TableScenario(ScenarioConfig& scenarioConfig, const DisplayConfig& displayConfig, int copies)
	: Scenario(scenarioConfig, displayConfig, copies)
{ }

void TableScenario::SaveLearningData(const string& file, const Tolerances& tolerances) const
{
	string data = "";
...
bool TableScenario::IsSuccessful(const Objects& objects, int idx, const Tolerances& tolerances) const
{
	...
	bool manipOnTable = abs(finalManipBox.Min.Z - finalStatic2Box.Min.Z) < tolerances.translationTolerance;
	bool movingBetweenStatic = IsBetween(finalManipBox, finalStatic1Box, finalStatic2Box, tolerances.translationTolerance);
	bool movingStraight = abs(finalManipPose.rot.Yaw) < tolerances.rotationTolerance || abs(finalManipPose.rot.Yaw - 360.0) < tolerances.rotationTolerance;
	...
	return (manipOnTable && static1SamePose && static2SamePose && movingBetweenStatic && movingStraight);
}

void TableScenario::UpdateTrials(TrialData& trialData, const Tolerances& tolerances)
{
	for (unsigned int i = 0; i<this->objects.size(); i++)
	{
