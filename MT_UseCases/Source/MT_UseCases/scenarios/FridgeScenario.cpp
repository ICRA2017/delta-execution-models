...
#include "FridgeScenario.h"

FridgeScenario::FridgeScenario(ScenarioConfig& scenarioConfig, const DisplayConfig& displayConfig, int copies)
	: Scenario(scenarioConfig, displayConfig, copies)
{ }

void FridgeScenario::SaveLearningData(const string& file, const Tolerances& tolerances) const
{
	string data = "";
...
bool FridgeScenario::IsSuccessful(const Objects& objects, int idx, const Tolerances& tolerances) const
{
	...
	bool manipOnShelf = abs(finalManipBox.Min.Z - finalStatic2Box.Min.Z) < tolerances.translationTolerance;
	...
	return (manipOnShelf && static1SamePose && static2SamePose && movingBetweenStatic);
}

void FridgeScenario::UpdateTrials(TrialData& trialData, const Tolerances& tolerances)
{
	for (unsigned int i = 0; i<this->objects.size(); i++)
	{
