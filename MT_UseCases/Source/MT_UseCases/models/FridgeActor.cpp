...
#include "FridgeActor.h"

AFridgeActor::AFridgeActor(const FObjectInitializer& objectInitialiser)
	: Super(objectInitialiser, TEXT("/Game/objects/fridge.fridge"))
{
	PrimaryActorTick.bCanEverTick = true;

	MeshParams meshParams(TEXT("Fridge"), FVector(0.07, 0.1, 0.13), true, true, 3);
	this->AddMesh(objectInitialiser, meshParams);
}

void AFridgeActor::BeginPlay()
{
	Super::BeginPlay();
}

void AFridgeActor::Tick(float DeltaTime)
{
