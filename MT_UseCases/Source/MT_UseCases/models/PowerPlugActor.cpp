...
#include "PowerPlugActor.h"

APowerPlugActor::APowerPlugActor(const FObjectInitializer& objectInitialiser)
	: Super(objectInitialiser, TEXT("/Game/objects/power_plug.power_plug"))
{
	PrimaryActorTick.bCanEverTick = true;

	MeshParams meshParams(TEXT("PowerPlug"), FVector(0.45, 0.46, 0.4), true, false, 0.4);
	this->AddMesh(objectInitialiser, meshParams);
}

void APowerPlugActor::BeginPlay()
{
	Super::BeginPlay();
}

void APowerPlugActor::Tick(float DeltaTime)
{
