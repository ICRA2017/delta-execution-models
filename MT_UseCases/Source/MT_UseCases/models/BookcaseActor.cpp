...
#include "BookcaseActor.h"

ABookcaseActor::ABookcaseActor(const FObjectInitializer& objectInitialiser)
	: Super(objectInitialiser, TEXT("/Game/objects/shelf.shelf"))
{
	PrimaryActorTick.bCanEverTick = true;

	MeshParams meshParams(TEXT("Bookcase"), FVector(2, 2, 2), true, true, 100);
	this->AddMesh(objectInitialiser, meshParams);
}

void ABookcaseActor::BeginPlay()
{
	Super::BeginPlay();
}

void ABookcaseActor::Tick(float DeltaTime)
{
