...
#include "ExtensionCordActor.h"

AExtensionCordActor::AExtensionCordActor(const FObjectInitializer& objectInitialiser)
	: Super(objectInitialiser, TEXT("/Game/objects/extension_cord.extension_cord"))
{
	PrimaryActorTick.bCanEverTick = true;

	MeshParams meshParams(TEXT("ExtensionCord"), FVector(0.5, 0.5, 0.5), true, true, 0.05);
	this->AddMesh(objectInitialiser, meshParams);
}

void AExtensionCordActor::BeginPlay()
{
	Super::BeginPlay();
}

void AExtensionCordActor::Tick(float DeltaTime)
{
